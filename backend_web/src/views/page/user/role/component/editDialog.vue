    ...
    <a-modal title="编辑职务">
        <a-form :model="props.item" name="basic" :label-col="{ span: 0 }" :wrapper-col="{ span: 16 }" autocomplete="off"
            ...
            <a-form-item label="职务名称" name="user_role_name" :rules="[{ required: true, message: '请输入职务名称!' }]">
                <a-input v-model:value="props.item.user_role_name" placeholder="请输入职务名称" />
            </a-form-item>
            <a-form-item label="所属用户绑定" name="bind_user_list">
                <a-select v-model:value="props.item.bind_user_list" :options="options" mode="multiple" :size="size"
                    placeholder="请选择所属用户绑定" style="width: 200px" @search="handleSearch"
                    :filter-option="false"></a-select>
            </a-form-item>
...
import { editUserRole } from '~/server/user/user_role';
...
const emit = defineEmits(['HandleEditSuccess']);
...
const onFinish = async (values: any): Promise<void> => {
    editUserRole.fetch({
        user_role_guid: props.item.user_role_guid,
        ...values
    }).then((res: editUserRole.returnResponse): void => {
        if (isRequestSuccess(res))
            emit("HandleEditSuccess", true)
    })
};
